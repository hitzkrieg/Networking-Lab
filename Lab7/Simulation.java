...
class Transfer /* Used to represent a connection. The sender and the receiver have their own versions of the Transfer object for the same connection */
	{
		...
		int w_max; /* window size before last timeout */
		double dev_RTT;
		...
		void update_source_window(double time)
		{
			double t = last_timeout - time;
			double c_cubic = ;
			double beta_cubic = ;
			double k_cubic = Math.pow(  (w_max * beta_cubic)/c_cubic , 1/3);
			source_window_size = (int)(Math.pow( c_cubic * (t - k_cubic) ,3) + w_max);
		}
	}
